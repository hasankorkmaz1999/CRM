...
export class AngularCalendarComponent implements OnInit {
  ...
  checkScreenWidth(): void {
    ...
    this.isListView = this.screenWidth <= 820; 
    if (this.isListView) {
      this.filterEvents(); 
    }
  }
  ...
  groupEventsByDate(): void {
    const grouped = this.filteredEvents.reduce(
      ...
      async (data) => {
        const eventsWithDetails = await Promise.all(
          data.map(async (eventData: any) => {
            ...
            const formattedUsers = await this.fetchUsersDetails(
              eventData.users || []
            );
            const creatorDetails = await this.fetchCreatorDetails(
              eventData.createdBy
            );
        ...
        this.events = eventsWithDetails.sort(
        ...
        try {
          ...
          if (!userSnapshot.empty) {
            ...
            return {
              ...
              profilePicture:
                userData['profilePicture'] || '/assets/img/user.png',
            };
          } else {
    ...
    return {
      ...
      meta: {
        ...
        createdBy: {
          ...
          profilePicture:
            creatorDetails['profilePicture'] || '/assets/img/user.png',
        },
